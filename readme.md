...
***注：为什么建议开启"自动更新并保持签到脚本活跃"工作流？***
根据Github Actions的规则，超过60天的仓库不进行任何活动，其actions会自动禁用，需要重新手动开启。开启此工作流后，每七天程序会自动对你fork的仓库执行一次空提交（如果有更新则提交更新内容），保持仓库处于活跃状态，这样actions就永远不会自动停用了～
